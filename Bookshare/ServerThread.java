...
public class ServerThread extends Thread {
	...
	/* 전송순서 저장 */
	//private static Hashtable<String, ServerThread> noteHash;
	//private static Vector<String> noteVector;
		...
		try {
			while (true) {
				...
				case REQ_SENDNOTE: {			
					...
					//	st_buffer.setLength(0);
					//	st_buffer.append(NO_SENDNOTE);
					//	send(st_buffer.toString());
					//} else {
						st_buffer.setLength(0);
						st_buffer.append(YES_SENDNOTE);
						st_buffer.append(SEPARATOR);
						// 전송한 사용자의 ID를 구한다.
						st_buffer.append(id);
						st_buffer.append(SEPARATOR);
						...
						st_buffer.append(data);
						st_buffer.append(SEPARATOR);
						st_buffer.append(uid);
						st_buffer.append(SEPARATOR);
						st_buffer.append(title);
						multicast(st_buffer.toString(), id, uid);
				//	}
					break;
				}
				// LOGOUT 전송 시도 메시지
	...
	private static synchronized void addUserName(String id, ServerThread client) {
